...
call :WriteState 12 CALCULATE_PARAMS "Calculating recovery partition size from HTA WinRE size."
call :CalculateRecoverySizeFromWinre
if errorlevel 1 (
    call :Fail 20 "Failed to calculate recovery partition size from HTA WinRE size."
    exit /b 20
)

call :WriteState 24 PRECLEAN_CHECK "Checking target disk before clean."
call :PrecheckBeforeClean
if errorlevel 1 (
    call :Fail 20 "Pre-clean checks failed. Disk will not be cleaned."
    exit /b 20
)
...
call :Log "Applying image index 1 from %WIM_PATH% to W:\."
>> "%APPLY_LOG%" echo.
>> "%APPLY_LOG%" echo Running DISM apply image.
dism /Apply-Image /ImageFile:"%WIM_PATH%" /Index:1 /ApplyDir:W:\ >> "%APPLY_LOG%" 2>&1
if errorlevel 1 (
...
set "WINRE_SIZE_BYTES=!HTA_WINRE_SIZE_BYTES!"
if not defined HTA_WINRE_SIZE_BYTES (
    call :Log "ERROR: HTA did not provide WINRE_SIZE_BYTES."
    exit /b 1
)
...
call :Log "Calculated recovery partition from HTA WinRE size: Path=!WINRE_PATH!, SizeBytes=!WINRE_SIZE_BYTES!, SizeMB=!WINRE_SIZE_MB!, RequiredMB=!WINRE_REQUIRED_MB!, RecoverySizeMB=!RECOVERY_SIZE_MB!."
exit /b 0

:PrecheckBeforeClean
call :Log "Running pre-clean checks for target disk."
call :PrecheckTargetDisk
if errorlevel 1 exit /b 1
call :Log "Pre-clean checks passed."
exit /b 0

:PrecheckTargetDisk
call :Log "Checking target disk %TARGET_DISK% before clean."
(
